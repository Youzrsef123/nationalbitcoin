...
dnl Output: If qt version is auto, set nationalbitcoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$nationalbitcoin_qt_want_version" = xauto && test "x$nationalbitcoin_qt_force" != xyes; then
    if test "x$nationalbitcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; nationalbitcoin-qt frontend will not be built])
    fi
    nationalbitcoin_enable_qt=no
    nationalbitcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$nationalbitcoin_enable_qt" != xno && test "x$nationalbitcoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build nationalbitcoin-qt GUI (default=auto)])],
    [
     nationalbitcoin_qt_want_version=$withval
     if test "x$nationalbitcoin_qt_want_version" = xyes; then
       nationalbitcoin_qt_force=yes
       nationalbitcoin_qt_want_version=auto
     fi
    ],
    [nationalbitcoin_qt_want_version=auto])
...
dnl Outputs: nationalbitcoin_enable_qt, nationalbitcoin_enable_qt_dbus, nationalbitcoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  BITCOIN_QT_CHECK([
    nationalbitcoin_enable_qt=yes
    nationalbitcoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      nationalbitcoin_enable_qt_test=no
    fi
    nationalbitcoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      nationalbitcoin_enable_qt_dbus=yes
    fi
  ...
  if test x$nationalbitcoin_enable_qt = xyes; then
    AC_MSG_RESULT([$nationalbitcoin_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$nationalbitcoin_enable_qt])
  fi
...
dnl Inputs: nationalbitcoin_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If nationalbitcoin_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: nationalbitcoin_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$nationalbitcoin_qt_want_version" = xauto; then
