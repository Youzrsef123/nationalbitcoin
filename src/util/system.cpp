...
const char * const BITCOIN_CONF_FILENAME = "nationalbitcoin.conf";
...
#else
    const char* pszModule = "nationalbitcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\National Bitcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\National Bitcoin
    // Mac: ~/Library/Application Support/National Bitcoin
    // Unix: ~/.nationalbitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "National Bitcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/National Bitcoin";
#else
    // Unix
    return pathRet / ".nationalbitcoin";
#endif
#endif
}
